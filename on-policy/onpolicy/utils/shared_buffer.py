...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if args.use_q_head:
            self.q_value_preds = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                                          dtype=np.float32)
            self.pi = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                               dtype=np.float32)
            self.ob = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, 1),
                               dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               q_value_preds, value_preds, pi, ob, rewards, masks, bad_masks=None, active_masks=None,
    ...
    def compute_returns(self, next_value, ob, value_normalizer=None):
        """
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart:
                        ...
                        delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(
                            self.value_preds[step + 1]) * self.masks[step + 1] - ob
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart:
                        delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(
                            self.value_preds[step + 1]) * self.masks[step + 1] \
                                - value_normalizer.denormalize(self.value_preds[step])
                        # delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
                    else:
                        delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - \
